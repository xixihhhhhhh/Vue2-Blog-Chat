...
export function intervalTime(startTime, endTime) {
  // 开始时间年月日
  const startYMD = startTime.split(' ')[0]
  // 结束时间年月日
  const endYMD = endTime.split(' ')[0]
  const startH = startTime.split(' ')[1]
  const endH = endTime.split(' ')[1]
  const startHh = (startH + '').split(':')[0]
  const endHh = (endH + '').split(':')[0]
  const startY = startYMD.split('-')[0]
  const startM = startYMD.split('-')[1]
  const startD = startYMD.split('-')[2]
  const endY = endYMD.split('-')[0]
  const endM = endYMD.split('-')[1]
  const endD = endYMD.split('-')[2]
  if (startY < endY) {
    return `${endY-startY}年前`
  } else if(startM < endM) {
    return `${endM - startM}月前`
  } else if(startD < endD){
    return `${endD - startD}天前`
  } else if (startHh < endHh) {
    return `${endHh - startHh}小时前`
  } else {
    return '不久前'
  }
}
